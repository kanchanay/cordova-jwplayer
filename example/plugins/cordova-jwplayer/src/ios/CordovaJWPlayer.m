...
- (void)createPlayer
{
    ...
    config.controls = self.options[JWPOptionState.controls];
    config.repeat =  self.options[JWPOptionState.repeat];
    config.premiumSkin = JWPremiumSkinRoundster;
    config.autostart = self.options[JWPOptionState.controls];
    
    NSLog(@" data %@", self.options);
    
    self.player = [[JWPlayerController alloc] initWithConfig:config];
    self.player.delegate = self;
  
    self.player.forceFullScreenOnLandscape = self.options[JWPOptionState.forceFullScreenOnLandscape];
    self.player.forceLandscapeOnFullScreen = self.options[JWPOptionState.forceLandscapeOnFullScreen];
